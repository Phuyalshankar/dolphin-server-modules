...
class DolphinClient {
    ...
    constructor(url = '', deviceId = '') {
        ...
        this._connectPromise = null; // Tracks in-flight connection attempt
    }
    ...
    async connect() {
        ...
        // 2. If connection is already in progress, return the existing promise
        ...
        this._connectPromise = new Promise((resolve, reject) => {
            ...
            this.socket.onopen = () => {
                ...
                this._heartbeatTimer = setInterval(() => {
                    if (this.socket && this.socket.readyState === WebSocket.OPEN) {
                        this.socket.send(JSON.stringify({ type: 'PING', from: this.deviceId }));
                    }
                }, 25000);
                resolve();
            };
    ...
    _handleMessage(data) {
        try {
            ...
            // १. Signaling Messages — server routes to correct device channel, trust delivery.
            const SIGNAL_TYPES = ['INVITE', 'ACCEPT', 'END', 'OFFER', 'ANSWER', 'ICE', 'ICE_CANDIDATE', 'ACK', 'BUSY', 'REJECT', 'CHAT'];
            if (parsed.type && parsed.from && SIGNAL_TYPES.includes(parsed.type)) {
    ...
    _maybeReconnect() {
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            // Jitter: wait 1s + random up to 2s to prevent storm
            const jitter = 1000 + (Math.random() * 2000);
            console.log(`[Dolphin] Reconnecting in ${Math.round(jitter)}ms (attempt ${this.reconnectAttempts})...`);
            setTimeout(() => this.connect().catch(() => {}), jitter);
        }
    }
}
