...
export class DolphinAgent {
    ...
    private chatHistory: { ts: string; role: 'user'|'assistant'; content: string }[] = [];

    constructor(config: AIConfig) {
        this.config = config;
        this.currentDir = process.cwd();
        const dolphinDir = path.join(this.currentDir, '.dolphin');
        this.historyFile = path.join(dolphinDir, 'history.json');
    ...
    private async loadHistory() {
        try {
            if (existsSync(this.historyFile)) {
                ...
                this.ai.history = this.chatHistory.slice(-20).map(h => ({ role: h.role, content: h.content }));
                CLIUI.success('📂 ' + this.chatHistory.length + ' past messages loaded (.dolphin/history.json)');
            }
        } catch { this.chatHistory = []; }
    }

    private async saveHistory(role: 'user'|'assistant', content: string) {
        try {
            const dir = path.dirname(this.historyFile);
            if (!existsSync(dir)) await fs.mkdir(dir, { recursive: true });
            this.chatHistory.push({ ts: new Date().toISOString(), role, content });
            if (this.chatHistory.length > 500) this.chatHistory = this.chatHistory.slice(-500);
            await fs.writeFile(this.historyFile, JSON.stringify(this.chatHistory, null, 2));
        } catch {}
    }

    private printHistory(n = 10) {
        const recent = this.chatHistory.slice(-n * 2);
        if (!recent.length) { console.log('  (history खाली छ)'); return; }
        recent.forEach(h => {
            const prefix = h.role === 'user' ? '\x1b[36m💬 You \x1b[0m' : '\x1b[32m🤖 AI  \x1b[0m';
            console.log(prefix + ' [' + h.ts.slice(0,16) + '] ' + h.content.slice(0, 120));
        });
    }
    ...
    public async interactiveChat() {
        ...
        while (true) {
            ...
            if (input.toLowerCase() === 'history') {
                this.printHistory(10);
                continue;
            }
            if (input.toLowerCase() === 'clear history') {
                this.chatHistory = []; this.ai.history = [];
                try { await fs.writeFile(this.historyFile, '[]'); } catch {}
                CLIUI.success('History cleared!');
                continue;
            }
            await this.saveHistory('user', input);
            ...
            while (isRunning && retryCount < MAX_RETRIES) {
                ...
                try {
                    ...
                    if (toolCalls.length > 0) {
                        ...
                        await this.saveHistory('assistant', response);
                        isRunning = false;
                    }
                } catch (e: any) {
                    console.log(`\n❌ API Error: ${e.message}`);
