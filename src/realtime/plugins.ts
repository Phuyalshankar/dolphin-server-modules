 ...
 */
function parseModbusFrame(buffer: Buffer): any {
  if (buffer.length < 4) {
 ...
 */
function registersToSensor(registers: number[], schema?: Record<string, { index: number, scale?: number, offset?: number }>): any {
  if (!schema) {
    // Default schema (temperature on register 0, humidity on register 1)
    return {
      temperature: registers[0] !== undefined ? (registers[0] as number) / 10 : null,
      humidity: registers[1] !== undefined ? (registers[1] as number) / 10 : null,
  ...
  for (const [key, config] of Object.entries(schema)) {
    ...
    if (config.scale) value = value * config.scale;
    if (config.offset) value = value + config.offset;
    result[key] = value;
  }
...
export const ModbusPlugin: RealtimePlugin = {
  ...
  match: (ctx) => {
    if (!ctx.raw || ctx.raw.length < 4) return false;
  ...
  onMessage: (ctx) => {
    ...
    // Log nicely
    console.log(`[Modbus] Slave ${parsed.slaveId} | ${parsed.functionName} | Data:`, 
      parsed.data.type === 'registers' ? parsed.data.registers : parsed.data);
  }
};
 ...
 */
export function buildWriteRegisterFrame(
  slaveId: number,
  address: number,
  value: number
): Buffer {
