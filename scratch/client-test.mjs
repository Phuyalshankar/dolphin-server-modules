import { DolphinClient } from '../scripts/client.js';

async function testClient() {
    console.log('🧪 Testing Dolphin Client (ESM)...');
    ...
    try {
        console.log('1. Testing API Proxy (client.api.products.get)...');
        const res = await client.api.products.get();
        console.log('✅ API GET Result:', res);

        console.log('2. Testing API POST...');
        const postRes = await client.api.products.post({ name: 'Test Product' });
