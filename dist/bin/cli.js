 ...
 */
function injectIntoAppJs(N, plural) {
 ...
 */
function injectIntoDbAdapter(N) {
    const candidates = [
    ...
    if (!alreadyInModels) {
        ...
        if (match) {
            ...
            src = src.replace(/(createMongooseAdapter\([\s\S]*?)(\}\s*\)\s*;)/, (_, body, close) => `${body}  ${N},\n${close}`);
            changed = true;
        }
    }
    if (changed) {
...
Return ONLY a JSON object: {filePath: fileContent}. No markdown.`;
        ...
        case 'init-prod': {
            ...
            const files = [
                ...
                ['adapters/connection.js', TEMPLATES.adaptersConnection],
                ['adapters/db.js', TEMPLATES.adaptersDbDecoupled],
        ...
        case 'add': {
            ...
            if (sub === 'adapter') {
                ...
                if (['mongoose', 'mongo', 'mongodb'].includes(type)) {
                    writeFile(path.join(adaptersDir, 'connection.js'), TEMPLATES.adaptersConnection, 'adapters/connection.js');
                    writeFile(path.join(adaptersDir, 'db.js'), TEMPLATES.adaptersDbDecoupled, 'adapters/db.js');
