import { createDolphinController, createNextPagesHandler } from './controller';
...
describe('Controller Factory', () => {
  ...
  it('maps standard controller methods to CRUD operations', async () => {
    const ctrl = createDolphinController(mockCrud, 'posts', { softDelete: false });
  ...
  it('maps next pages handler correctly', async () => {
    const ctrl = createDolphinController(mockCrud, 'posts');
    const handler = createNextPagesHandler(ctrl);
